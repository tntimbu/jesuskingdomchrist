...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Featured Social Media Video Player Widget */}
        ...
        <div className={`rounded-3xl ${cardStyleClass} p-6 overflow-hidden text-white space-y-4`}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-white/10">
            ...
            {isAdmin && (
              <button
                onClick={() => setIsCustomizerOpen(true)}
                className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-indigo-300 text-xs font-semibold border border-indigo-500/30 flex items-center gap-1.5 shrink-0"
              >
                <Video className="w-3.5 h-3.5 text-indigo-400" />
                <span>Ganti Link Video</span>
              </button>
            )}
          </div>
            ...
            {parsedVideo.isValid ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center text-slate-400">
                ...
                <p className="text-xs mt-1 max-w-md">
                  SuperAdmin / Admin dapat menempelkan link video YouTube, Shorts, Instagram Reel, TikTok, atau MP4 melalui tombol Kustomisasi.
                </p>
                {isAdmin && (
                  <button
                    onClick={() => setIsCustomizerOpen(true)}
                    className="mt-4 px-4 py-2 rounded-xl bg-indigo-600 text-white font-bold text-xs shadow-lg"
                  >
                    Tempelkan Link Video Sekarang
                  </button>
                )}
              </div>
            )}
